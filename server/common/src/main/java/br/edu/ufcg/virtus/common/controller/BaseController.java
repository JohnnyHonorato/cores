...
import br.edu.ufcg.virtus.common.dto.RestMessageDTO;
...
public abstract class BaseController {
   ...
   @Override
    ...
    protected <E> ResponseEntity<E> ok(E element) {
        return this.responseEntity(element, HttpStatus.OK);
    }
    ...
    protected <E> ResponseEntity<E> notAcceptable(Exception e) {
        return this.notAcceptable(e.getMessage());
    }
    ...
    protected <E> ResponseEntity<E> notAcceptable(String message) {
        return new ResponseEntity<>((E) new RestMessageDTO(MessageUtil.findMessage(message, LocaleContextHolder.getLocale())),
     ...
     */
    protected <D, S> Set<D> toSet(Set<S> items, Class<D> destClass) {
        return MapperUtil.toSet(items, destClass);
    }
    ...
    protected String getPathVariable(HttpServletRequest request, String variable) {
        final ResourceUrlProvider urlProvider = (ResourceUrlProvider) request.getAttribute(ResourceUrlProvider.class.getCanonicalName());

        final Map<String, String> params = urlProvider.getPathMatcher().extractUriTemplateVariables(
